...
class Supervisor:
    ...
    def handle_sensor_data(self, topic, json_data):
        ...
        event_ref = db.reference(f'test4/events/sensors/{sensor.id}/{id}')
        ...
        if alarm is not None:
            ...
            for id in sensors_to_check:
                sensor_to_check = self.config.get_sensor(id)
                G.add_node(f'S_{id}', label=sensor_to_check.name, id=id, color='blue')
            # Add fault nodes to the graph
            for fault in possible_faults:
                ...
                for symptom in fault.symptoms:
                    symptom_sensor = self.config.get_sensor(symptom.sensor_id)
                    # normalize the probability by the number of symptoms
                    probability = round(symptom_sensor.get_symptom_probability(symptom) / len(fault.symptoms), 2)
                ...
                fault_ref = db.reference(f'sensor-faults/{sensor.id}/{fault.id}')
                fault_ref_2 = db.reference(f'test4/sensor-faults/{sensor.id}/{fault.id}')
                fault_ref.set(fault_event)
                fault_ref_2.set(fault_event)
